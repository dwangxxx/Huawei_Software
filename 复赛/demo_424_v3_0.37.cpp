...
const int nodes_num = 4500000;
const int max_degress = 201;
...
int graph[nodes_num][max_degress] = {{0}};
int g_in[nodes_num][max_degress] = {{0}};
int in_degrees[51000] = {0};
int out_degrees[51000] = {0};
...
string id_comma[nodes_num];
string id_end[nodes_num];
int nodes_cnt = 0;

static_vector<int, nodes_num> inputs;
...
public:
    ...
    void Input_Mmap(const string &test_file)
    {
        ...
        for(int i = 0; i < len; ++i)
        {
            ...
            {
                tmp[cnt] = tmp[cnt] * 10 + data_ptr[i] - '0';
                judge = false;
            }
            else
            {
                ...
                {
                    if(tmp[0] > 50000 || tmp[1] > 50000)    
                    {
                        continue;
                    }
                    inputs.push_back(tmp[0]);
                    inputs.push_back(tmp[1]);
                }
            }
        }
    }
    ...
    static void DFS(int head, int cur, int *cur_path, int depth, int thread_id, int *reachable, bool *visited){
        visited[cur] = true;
        cur_path[depth - 1] = cur;
        for(int i = 0; i < out_degrees[cur]; i++){
            int v = graph[cur][i];
            // 如果当前点小于head或者其不能到达head, 则不进行DFS
            if (v < head || (depth > 3 && reachable[v] != head && reachable[v] != -2)) continue;
            // 如果当前节点能直接到达head, 则有环
            else if (reachable[v] == -2 && !visited[v] && depth >= 2) {
                cur_path[depth] = v;
                if(depth == 6)
                {
            ...
            if (depth < 6 && !visited[v]) {
                DFS(head, v, cur_path, depth + 1, thread_id, reachable, visited);
            }
        }
        visited[cur] = false;
    }
    ...
    static void CheckCircle(int thread_id) {
        ...
        for (int i = thread_id; i < nodes_cnt; i += 4){
            ...
            DFS(i, i, cur_path, 1, thread_id, reachable, visited);
...
int main()
{
    ...
    cout<<"Time of DFS: "<<(double) (us2.tv_sec - us1.tv_sec) * 1000000 + (us2.tv_usec - us1.tv_usec)<<"us"<<endl;
    s.DFS_thread();
