...
private:
    ...
    const double stepSize = 0.09;
    const int maxIterTimes = 500;
    const double predictTrueThresh = 0.45;
    const int train_show_step = 800;
};
...
void LR::train()
{
    ...
    int i, j;
    int remainder;
    int length = trainDataSet.size() / 2;

    for (i = 0; i < maxIterTimes; i++) {
        vector<double> sigmoidVec;
        
        remainder = i % 2;
        for (j = length * remainder; j < length * (remainder + 1); j++) {
