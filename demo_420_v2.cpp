...
public:
    ...
    void DFS_v2(int n, int head, vector<int> &stack, int depth, bool *visited)
    {
        ...
        if(depth < 6)
        {
            ...
            {
                for(int i = 0; i < graph[n][0]; i++)
                    if(!visited[graph[n][i + 1]] && graph[n][i + 1] > head)
                    {
                        //if(depth == 4 && !reach[stack[0]].count(graph[n][i + 1]))
                        if(depth == 5 && reach[stack[0]][graph[n][i + 1]] == 0)
                            continue;    
                        DFS_v2(graph[n][i + 1], head, stack, depth + 1, visited);
                    }
            }
        }
        // 处理长度为7的环
    ...
    void BFS()
    {
        ...
        for(int k = 0; k < nodes_cnt; k++)//从第一个节点开始进行bfs
        {
            if(graph[k][0] != 0)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    int *que_new = new int[51 * cnt_que];
                    cnt_que_new = 0;
                    for (int j = 0; j < cnt_que; j++)//遍历当前que中有的元素
                    {
                        int cur = que[j];
                        if(graph[cur][0] != 0){ //如果当前节点已经在邻接表中
                            for(int m = 1; m <= graph[cur][0]; m++){ //从小到大的遍历它的后继元素
                                int each_node = graph[cur][m];
                                que_new[cnt_que_new++] = each_node;
                                if(each_node < k && each_node < cur)
    ...
    void CreateGraph(string test_file)
    {
        ...
        reach = new int*[nodes_cnt];
        for(i = 0; i < nodes_cnt; i++)
        {
            reach[i] = new int[nodes_cnt];
            if(graph[i][0] != 0)
            {
...
int main()
{
    ...
    //cout<<"Time of DFS: "<<(double)(clock() - start) / CLOCKS_PER_SEC<<"s"<<endl;
    exit(0);
}